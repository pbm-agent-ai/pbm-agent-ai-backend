 ...
 *
 * Product 테이블 제거 후 MonitorTarget이 메타데이터를 직접 가지는 구조를 검증한다.
...
class ProductPersistenceServiceTest {
    ...
    @Test
    @DisplayName("네이버 검색 결과 저장 - 신규 상품이면 MonitorTarget과 PriceHistory가 생성된다")
    void saveNaverSearchResults_createsEntitiesForNewProduct() {
        ...
        MonitorTarget target = monitorTargetRepository.findAll().get(0);
        assertThat(target.getPlatform()).isEqualTo(Platform.NAVER);
        assertThat(target.getProductId()).isEqualTo("naver-1001");
        assertThat(target.getTitle()).isEqualTo("아이폰 15");
        assertThat(target.getCategoryPath()).isEqualTo("디지털/휴대폰/스마트폰");
        assertThat(target.getMallName()).isEqualTo("애플공식몰");
        ...
        assertThat(priceHistory.getMonitorTarget().getId()).isEqualTo(target.getId());
    }

    @Test
    @DisplayName("같은 상품 재조회 - MonitorTarget은 재사용하고 PriceHistory는 누적 저장한다")
    void saveNaverSearchResults_reusesMonitorTargetAndAccumulatesHistory() {
        // given
        ...
        // then: MonitorTarget은 1개만 존재 (재사용), PriceHistory는 2개 누적
        assertThat(monitorTargetRepository.findAll()).hasSize(1);
        assertThat(priceHistoryRepository.findAll()).hasSize(2);

        MonitorTarget target = monitorTargetRepository.findAll().get(0);
        assertThat(target.getTitle()).isEqualTo("아이폰 15 최신");
        assertThat(target.getProductUrl()).isEqualTo("https://shopping.example.com/iphone15-new");
    }
    ...
    void saveAliExpressSearchResults_usesCategoryPathFromCategoryNodes() {
        ...
        MonitorTarget target1 = monitorTargetRepository.findByPlatformAndProductId(Platform.ALIEXPRESS, "ali-1")
                .orElseThrow();
        assertThat(target1.getPlatform()).isEqualTo(Platform.ALIEXPRESS);
        assertThat(target1.getProductId()).isEqualTo("ali-1");
        assertThat(target1.getSearchKeyword()).isEqualTo("아이폰");
        // second_level_category_id에 매핑된 category_nodes의 categoryPath를 우선 사용
        assertThat(target1.getCategoryPath()).isEqualTo("전자제품/액세서리/휴대폰케이스");
    }
    ...
    void saveAliExpressSearchResults_fallsBackToJoinedCategoryNames_whenCategoryMappingMissing() {
        ...
        MonitorTarget target = monitorTargetRepository.findAll().get(0);
        assertThat(target.getCategoryPath()).isEqualTo("케이스/휴대폰케이스");
    }
}
