...
@Table(
        ...
        uniqueConstraints = @UniqueConstraint(
                name = "uk_monitor_targets_source_keyword",
                columnNames = {"source_type", "normalized_keyword"}
        )
)
...
public class MonitorTarget {
    ...
    private MonitorTarget(SourceType sourceType, String normalizedKeyword, Integer fetchIntervalMinutes) {
     ...
     * 새 공통 수집 대상을 생성한다.
     *
     * @param sourceType           플랫폼 구분(enum)
     * @param normalizedKeyword    공백/대소문자를 정리한 공유 키워드
     * @param fetchIntervalMinutes 기본 수집 주기(분)
     * @return 생성된 MonitorTarget 엔티티
     */
    public static MonitorTarget create(SourceType sourceType, String normalizedKeyword, Integer fetchIntervalMinutes) {
        return new MonitorTarget(sourceType, normalizedKeyword, fetchIntervalMinutes);
    }
